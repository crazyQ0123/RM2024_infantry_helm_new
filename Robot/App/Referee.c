...
ext_dart_info_t       					Dart_Remaining_Time;

/* 0x020X */
ext_robot_status_t 							Game_Robot_Status;
ext_power_heat_data_t 					Power_Heat_Data;
ext_robot_pos_t   							Game_Robot_Pos;
ext_buff_t        							Buff_Musk;
air_support_data_t  						Aerial_Robot_Energy;
ext_hurt_data_t       					Robot_Hurt;
ext_shoot_data_t       					Shoot_Data;
ext_projectile_allowance_t		  Bullet_Remaining;
ext_rfid_status_t      					RFID_Status;

/* 0x030X */
ext_robot_interactive_header_data_t 	 	Robot_Interactive_Header_Data;
robot_interactive_data_t Robot_Interactive_Data;
ext_map_command_t              	Map_Command_Data;
...
void Referee_StructInit(void)
{
	...
	memset(&Game_Robot_Status,               0, sizeof(Game_Robot_Status));
	...
	memset(&Robot_Interactive_Header_Data, 				 0, sizeof(Robot_Interactive_Header_Data));
	memset(&Robot_Interactive_Data, 				 0, sizeof(Robot_Interactive_Data));
	memset(&Map_Command_Data,          			 0, sizeof(Map_Command_Data));
...
void Referee_SolveFifoData(uint8_t *frame)
{
    ...
    case FIELD_EVENTS_CMD_ID:              memcpy(&Event_Data,                frame + index, sizeof(ext_event_data_t));                break;
		...
		case DART_SHOOT_RELEVANT_DATA_CMD_ID:  memcpy(&Dart_Remaining_Time,       frame + index, sizeof(ext_dart_info_t));       					 break;
		
		case ROBOT_STATE_CMD_ID:               memcpy(&Game_Robot_Status,         frame + index, sizeof(ext_robot_status_t));          		 break;
		case POWER_HEAT_DATA_CMD_ID:           memcpy(&Power_Heat_Data,           frame + index, sizeof(ext_power_heat_data_t));           break;
		case ROBOT_POS_CMD_ID:                 memcpy(&Game_Robot_Pos,            frame + index, sizeof(ext_robot_pos_t));            		 break;
		case BUFF_MUSK_CMD_ID:                 memcpy(&Buff_Musk,                 frame + index, sizeof(ext_buff_t));                 		 break;
		case AERIAL_ROBOT_ENERGY_CMD_ID:       memcpy(&Aerial_Robot_Energy,       frame + index, sizeof(air_support_data_t));           	 break;
		case ROBOT_HURT_CMD_ID:                memcpy(&Robot_Hurt,                frame + index, sizeof(ext_hurt_data_t));                 break;
		case SHOOT_DATA_CMD_ID:                memcpy(&Shoot_Data,                frame + index, sizeof(ext_shoot_data_t));                break;
		case BULLET_ALLOWANCE_CMD_ID:          memcpy(&Bullet_Remaining,          frame + index, sizeof(ext_projectile_allowance_t));      break;
		case ROBOT_RFID_STATE_CMD_ID:          memcpy(&RFID_Status,               frame + index, sizeof(ext_rfid_status_t));               break;
		
		case ROBOT_INTERACTIVE_DATA_CMD_ID:  	 memcpy(&Robot_Interactive_Data,    frame + index, sizeof(robot_interactive_data_t));    		 break;
		case CLIENT_MAP_INTERACTIVE_CMD_ID:    memcpy(&Map_Command_Data,          frame + index, sizeof(ext_map_command_t));             	 break;
		case CLIENT_MAP_RECIEVE_REDAR_CMD_ID:  memcpy(&Map_Robot_Data,        		frame + index, sizeof(ext_map_robot_data_t));        		 break;
		case CLIENT_MAP_RECIEVE_ROBOT_CMD_ID:  memcpy(&Custom_Info,        				frame + index, sizeof(ext_map_robot_data_t));        		 break;
										...
										char                 *StringData)    //字符串内容
{
	...
	String->string_tpye   = UI_Graph_String;
	String->layer          = Layer;
	String->color          = Color;
	String->font_size    = CharSize;
...
void UI_PushUp_Graphs(uint8_t Counter /* 1,2,5,7 */, void *Graphs /* 与Counter相一致的UI_Graphx结构体头指针 */, uint8_t RobotID)
{
	...
	/* 填充 cmd_id */
	Graph->CMD_ID = ROBOT_INTERACTIVE_DATA_CMD_ID;
...
void UI_PushUp_String(UI_String_t *String, uint8_t RobotID)
{
	...
	/* 填充 cmd_id */
	String->CMD_ID = ROBOT_INTERACTIVE_DATA_CMD_ID;
...
void UI_PushUp_Delete(UI_Delete_t *Delete, uint8_t RobotID)
{
	...
	/* 填充 cmd_id */
	Delete->CMD_ID = ROBOT_INTERACTIVE_DATA_CMD_ID;
