...
typedef struct __attribute__((packed))  //0x0102 补给站动作标识
{
	uint8_t reserved;
...
typedef struct __attribute__((packed))  //0x0104 裁判警告信息
{
	uint8_t level;
...
typedef struct __attribute__((packed))  //0x0105 飞镖发射口倒计时
{
...
/* 0x020X --------------------------------------------------------------------*/
typedef struct __attribute__((packed))  //0x0201 比赛机器人状态
{
	uint8_t robot_id;
	uint8_t robot_level;
	uint16_t current_HP; 
...
typedef struct __attribute__((packed))  //0x0202 实时功率热量数据
{
...
typedef struct __attribute__((packed))  //0x0203 机器人位置
{
	float x;
	float y;
	float angle;
...
typedef struct __attribute__((packed))  //0x0204 机器人增益
{
...
typedef struct __attribute__((packed))  //0x0205 空中机器人能量状态
{
	uint8_t airforce_status;
	uint8_t time_remain;
} air_support_data_t;

typedef struct __attribute__((packed))  //0x0206 伤害状态
{
	uint8_t armor_id : 4;
	uint8_t HP_deduction_reason : 4;
} ext_hurt_data_t;

typedef struct __attribute__((packed))  //0x0207 实时射击信息
{
	uint8_t bullet_type;
...
typedef struct __attribute__((packed))  //0x0208 子弹剩余发射数
{
...
typedef struct __attribute__((packed))  //0x0209 机器人RFID状态
{
	uint32_t rfid_status;
} ext_rfid_status_t;

/* 0x030X --------------------------------------------------------------------*/
typedef struct __attribute__((packed))  //0x0301 机器人间通信 头结构体
{
...
typedef struct __attribute__((packed))  //0x0301 机器人间通信 数据结构体
{
	uint8_t *data;
} robot_interactive_data_t;

typedef struct __attribute__((packed))  //0x0303 小地图下发信息标识
{
...
typedef struct __attribute__((packed))  //0x0305 小地图接收信息标识
{
...
typedef struct __attribute__((packed))  //0x0308 选手端小地图接收机器人数据
{
	uint16_t sender_id;
	uint16_t receiver_id;
...
/* 自定义绘制UI结构体 -------------------------------------------------------*/
typedef struct __attribute__((packed))  //绘制UI UI图形数据
{
...
typedef struct __attribute__((packed))  //绘制UI UI字符串数据
{
	uint8_t  string_name[3];
	uint32_t operate_tpye:3;
	uint32_t string_tpye:3;
	uint32_t layer:4;
	uint32_t color:4;
	uint32_t font_size:9;
	uint32_t string_length:9;
	uint32_t width:10;
	uint32_t start_x:11;
	uint32_t start_y:11;
	uint32_t null;
...
typedef struct __attribute__((packed))  //绘制UI UI删除图形数据
{
	uint8_t operate_tpye;
	uint8_t layer;
} delete_data_struct_t;

typedef struct __attribute__((packed)) //绘制UI 绘制1个图形完整结构体
{
	frame_header_struct_t Referee_Transmit_Header;
	uint16_t CMD_ID;
	ext_robot_interactive_header_data_t Interactive_Header;
	graphic_data_struct_t Graphic[1];
	uint16_t CRC16;
} UI_Graph1_t;

typedef struct __attribute__((packed)) //绘制UI 绘制2个图形完整结构体
{
	frame_header_struct_t Referee_Transmit_Header;
	uint16_t CMD_ID;
	ext_robot_interactive_header_data_t Interactive_Header;
...
typedef struct __attribute__((packed)) //绘制UI 绘制5个图形完整结构体
{
	frame_header_struct_t Referee_Transmit_Header;
	uint16_t CMD_ID;
	ext_robot_interactive_header_data_t Interactive_Header;
...
typedef struct __attribute__((packed)) //绘制UI 绘制7个图形完整结构体
{
	frame_header_struct_t Referee_Transmit_Header;
	uint16_t CMD_ID;
	ext_robot_interactive_header_data_t Interactive_Header;
...
typedef struct __attribute__((packed)) //绘制UI 绘制1字符串完整结构体
{ 
	frame_header_struct_t Referee_Transmit_Header;
	uint16_t CMD_ID;
	ext_robot_interactive_header_data_t Interactive_Header;
...
typedef struct __attribute__((packed))  //绘制UI UI删除图形完整结构体
{
	frame_header_struct_t Referee_Transmit_Header;
	uint16_t CMD_ID;
	ext_robot_interactive_header_data_t Interactive_Header;
